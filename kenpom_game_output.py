...
Uses Kenpom efficiency ratings to predict game scores
...
# D1 Averages (updated for current season)
D1_AVERAGES = {
    'tempo': 67.7,  
    'def_efficiency': 111.0,  
}
...
def calculate_tempo(kenpom_data: pd.DataFrame, home_team: str, away_team: str) -> float:
    """Calculate expected tempo for the game"""
    home_data = get_team_data(kenpom_data, home_team)
    away_data = get_team_data(kenpom_data, away_team)
    
    tempo = (home_data['AdjT'] * away_data['AdjT']) / D1_AVERAGES['tempo']
    return tempo


def calculate_home_score(kenpom_data: pd.DataFrame, home_team: str, away_team: str, neutral_site: bool) -> float:
    """Calculate predicted home team score"""
    home_data = get_team_data(kenpom_data, home_team)
    away_data = get_team_data(kenpom_data, away_team)
    
    # Get tempo
    tempo = calculate_tempo(kenpom_data, home_team, away_team)
    ...
    if not neutral_site:
        # Check if HCA column exists, otherwise use default 3.5
        hca = float(home_data['HCA']) if 'HCA' in home_data.index else 3.5
        score += hca
    
    return score


def calculate_away_score(kenpom_data: pd.DataFrame, away_team: str, home_team: str) -> float:
...
def predict_games():
    ...
    print("="*60)
    print("KENPOM GAME PREDICTIONS")
    print("="*60)
    
    # Load data
    try:
        kenpom_data = pd.read_csv('kenpom_data.csv')
        matchups_df = pd.read_csv('daily_matchups.csv')
    except FileNotFoundError as e:
        print(f"ERROR: {e}")
        print("Run fetch_kenpom_data.py and fetch_daily_matchups.py first")
        return None
    ...
    for _, game in matchups_df.iterrows():
        ...
        try:
            ...
            home_score = calculate_home_score(kenpom_data, home_team, away_team, neutral_site)
            away_score = calculate_away_score(kenpom_data, away_team, home_team)
    ...
    # Save to CSV
    output_file = f'kenpom_predictions.csv'
